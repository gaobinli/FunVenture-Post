...
<template>
  <div class="layout-container">
    <el-form :model="form" :rules="rules" ref="ruleForm" label-width="120px" style="margin-right:30px;">
      ...
      <el-form-item label="酒店封面：" prop="url">
        <el-upload
          class="upload-demo"
          drag
          :action="uploadPath"
...
import { dataList, del } from '@/api/back/examinationPaper'
...
import { add, update, one } from '@/api/back/hotel'
    ...
    function init() { // 用于判断新增还是编辑功能
      ...
      if(id) {
        ...
        .then(res => {
          ...
          form.value.shortRecommend = res.data.shortRecommend
          form.value.address = res.data.address
          form.value.inventory = res.data.inventory
          form.value.library = res.data.library
        })
      }
    }
    const close = () => {
      router.push({path:'/hotel/hotel'});
    }
    const addForm = () => {
      if (ruleForm) {
