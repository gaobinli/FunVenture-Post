...
import { getHotelAPI } from '@/services/hotel'
...
const getHomeScenicSpotRecommendData = async () => {
  ...
  const res = await getHotelAPI(params)
  // 数组追加
...
<template>
  <view class="guess">
    <navigator class="guess-item" v-for="item in hotelList" :key="item.id"
      :url="`/pages/hotel/hotelDetail?id=${item.id}`">
