...
import { getScenicSpotByIdAPI } from '@/services/scenicSpot'
...
const scenicSpots = ref<ScenicSpotItem[]>([])
const scenicSpot = ref<ScenicSpotItem>()
const imgs = ref([])
const userInfo = ref<any>(null)

// 获取景点信息
const getScenicSpotByIdData = async () => {
  const res = await getScenicSpotByIdAPI(query.id)
    ...
    success: async (res) => {
      if (res.confirm) {
        try {
          ...
          // 调用购买接口
          await addOrderAPI({ relationId: id, type: 1 })
...
const coolect = (id: number, name: string) => {
  ...
  uni.showModal({
    ...
    success: (res) => {
      if (res.confirm) {
        console.log("确定了", id)
        const res = addCollectAPI({ relationId: id, type: 1 })
        console.log("res", res)
        if (memberStore.profile) {
...
<template>
  <scroll-view scroll-y class="viewport">
    ...
    <view class="goods">
      ...
      <view class="preview">
        ...
        <view class="indicator">
          ...
          <text class="total">{{ imgs.length }}</text>
        </view>
      </view>
      <view class="meta">
        <view class="price">
          <text class="number">{{ scenicSpot?.name }}</text>
        </view>
        <view class="name ellipsis">{{ scenicSpot?.shortRecommend }}</view>
      </view>
      ...
      <view class="operation">
        <view class="item">
          ...
          <text class="text ellipsis"> {{ scenicSpot?.price }} </text>
        </view>
        <view class="item">
          <text class="label">地址</text>
          <text class="text ellipsis"> {{ scenicSpot?.address }} </text>
        </view>
        <view class="item">
    ...
    <view class="detail panel">
      ...
      <view class="content">
        ...
        <rich-text :nodes="scenicSpot?.introduce"></rich-text>
      </view>
    </view>
  ...
  <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    ...
    <view class="buttons">
      <view class="addcart" @tap="buy(scenicSpot!.id, scenicSpot!.price)"> 门票购买 </view>
      <view class="payment" @tap="coolect(scenicSpot!.id, scenicSpot!.name)"> 加入收藏 </view>
    </view>
  </view>
</template>
