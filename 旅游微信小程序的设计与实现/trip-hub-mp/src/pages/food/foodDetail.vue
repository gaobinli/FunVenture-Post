...
import { getFoodByIdAPI } from '@/services/food'
...
const getFoodByIdData = async () => {
  const res = await getFoodByIdAPI(query.id)
  food.value = res.data
    ...
    success: async (res) => {
      if (res.confirm) {
        try {
          ...
          // 调用购买接口
          await addOrderAPI({ relationId: id, type: 2 })
...
const coolect = (id: number, name: string) => {
  ...
  uni.showModal({
    ...
    success: (res) => {
      if (res.confirm) {
        console.log("确定了", id)
        const res = addCollectAPI({ relationId: id, type: 2 })
        console.log("res", res)
        if (memberStore.profile) {
...
<template>
  <scroll-view scroll-y class="viewport">
    <view class="goods">
      <view class="preview">
        <swiper @change="onchange" circular>
          <swiper-item v-for="item in foods" :key="item">
      ...
      <view class="meta">
        <view class="price">
          <text class="number">{{ food?.name }}</text>
        </view>
        <view class="name ellipsis">{{ food?.shortRecommend }}</view>
      </view>
      <view class="operation">
        <view class="item">
          <text class="label">门票</text>
          <text class="text ellipsis"> {{ food?.price }} </text>
        </view>
        <view class="item">
          <text class="label">地址</text>
          <text class="text ellipsis"> {{ food?.address }} </text>
        </view>
        <view class="item">
    ...
    <view class="detail panel">
      ...
      <view class="content">
        <rich-text :nodes="food?.content"></rich-text>
      </view>
    </view>
  </scroll-view>
  <!-- 用户操作 -->
  <view class="toolbar" :style="{ paddingBottom: safeAreaInsets?.bottom + 'px' }">
    ...
    <view class="buttons">
      <view class="addcart" @tap="buy(food!.id, food!.price)"> 美食购买 </view>
      <view class="payment" @tap="coolect(food!.id, food!.name)"> 加入收藏 </view>
    </view>
  </view>
</template>
